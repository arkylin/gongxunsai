...
def vision_block(conn1,conn2):    
    ...
    if cap.isOpened():
        ...
        while True:
            ...
            mask = cv2.rectangle(mask,(frame_wh[0]-60,frame_wh[1]-100),(frame_wh[0],frame_wh[1]),(0,0,0),-1)
            ...
            mask_copy = mask.copy()
            hfill, wfill = mask_copy.shape[:2]
            maskfill = np.zeros((hfill+2, wfill+2), np.uint8)

            cv2.floodFill(mask_copy, maskfill, (0,0), 255);
            cv2.imshow("TT",mask_copy)
